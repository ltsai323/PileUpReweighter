...
import ROOT
import PileUpReweighter.PileUpReweighter.InfoLoader as InfoLoader
...
def checkInputs(hist_data, mc_vallist):
    ...
    if len(mc_vallist) < hist_data.GetNbinsX():
        raise IOError('input MC binning is smaller than data binning. Check needed...')

def RenameOutput(outputname):
    import os
    os.system( 'mv tmpPUoutput.root %s' % (origname,outname) )
    mylog.info('output file : %s' % outname)
if __name__ == '__main__':
    LogMgr.InitLogger(level='info')
    mylog=LogMgr.GetLogger(__name__)

    mcInfos=InfoLoader('data/2016ReReco_Moriond17.json')
    ...
    for iFile, oFile in mcInfos.GetIOFiles():
        ...
        fout=ROOT.TFile('_PUoutput.root','recreate')
        fout.cd()
